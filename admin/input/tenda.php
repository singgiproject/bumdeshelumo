...
<head>
  ...
  <meta name="description" content="Aplikasi Badan Usaha Milik Desa (BUMDES)" />
	...
	<title>Input - 
		<?php foreach( $unitTenda as $row ) : ?> <?= $row["nama_unit"]; ?> <?php endforeach; ?>
	</title>
					...
					<div class="table-data-members">
						...
						<table>
							<tr>
								...
								<th>Gambar</th>
								<th>Nama Unit</th>
								<th>Harga Dalam Desa</th>
								<th>Harga Luar Desa</th>
								<th>Stok</th>
								<th>Aksi</th>
							</tr>
							...
							<?php $id_no = 1; ?>
								...
								<tr>
									<td><?= $id_no; ?></td>
									<td>
										<a href="../../assets/images/kanopi/<?= $row["gambar_unit"] ?>">
											<img src="../../assets/images/kanopi/<?= $row["gambar_unit"] ?>" alt="LPG">
										</a>
									</td>
									<td><?= $row["nama_unit"]; ?></td>
									<td>Rp. <?= number_format($row["harga_unit_1"], 0, ',', '.'); ?></td>
									<td>Rp. <?= number_format($row["harga_unit_2"], 0, ',', '.'); ?></td>
									<td><?= $row["stok_unit"]; ?></td>
									<td>
										<table>
											<tr>
												<td>
													<a href="update/tenda?tenda=<?= base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode($row["id_tenda"])))))); ?>" class="btn-warning" title="Ubah"><i class="fa fa-edit"></i></a>
												</td>
											</tr>
										</table>
									</td>
