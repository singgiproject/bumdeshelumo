...
$id_tenda = base64_decode(base64_decode(base64_decode(base64_decode(base64_decode(base64_decode($_GET["tenda"]))))));
$unitTenda = query("SELECT * FROM tb_tenda WHERE id_tenda = '$id_tenda' ")[0];


// URL SECURITY TAMBAHAN 
if( !isset($_GET["tenda"]) ){
	header("Location:../tenda");
	return false;
}
if( empty($_GET["tenda"]) ){
	header("Location:../tenda");
...
if( strlen($_GET["tenda"]) < 12 ){
	header("Location:../tenda");
	return false;	
}
if( !base64_decode(base64_decode(base64_decode(base64_decode(base64_decode(base64_decode($_GET["tenda"])))))) ){
	header("Location:../tenda");
	return false;	
}

if( isset($_POST["update_tenda"]) ){
	if( updateTenda($_POST) > 0 ){
		echo "
		<script>
			alert('Tenda Berhasil diubah!');
...
<head>
  ...
  <meta name="description" content="Aplikasi Badan Usaha Milik Desa (BUMDES)" />
	...
	<title>Ubah Data <?= $unitTenda["nama_unit"]; ?></title>
...
<body>
 ...
 -->				<li><a href="../../logout" onclick="return confirm('Pilih Oke jika anda siap untuk mengakhiri sesi anda saat ini.');"><i class="fa fa-sign-out"></i> Keluar</a></li>
		...
		<div class="container-content">
			<div class="header">
				<h2>Ubah Data <?= $unitTenda["nama_unit"]; ?></h2>
			</div>
			...
			<div class="container-data-members">
				<div class="card-data-members">
					<div class="menu-data-members">
						<ul>
							<li><a href="../tenda"><i class="fa fa-angle-left"></i> Kembali</a></li>
						</ul>
					</div>
					<div class="table-form-members">
						<header>Form Ubah Data <?= $unitTenda["nama_unit"]; ?></header>

						<fieldset>
							<legend>Form <?= $unitTenda["nama_unit"]; ?></legend>
							<form action="" method="post" enctype="multipart/form-data">
								<input type="hidden" name="id_tenda" value="<?= $unitTenda["id_tenda"]; ?>">
								<input type="hidden" name="gambarLama" value="<?= $unitTenda["gambar_unit"]; ?>">
								<button type="submit" name="update_tenda" class="btn-green-gradient pd-10 btn-post-members" onclick="return confirm('Publikasi sekarang?');"><i class="fa fa-save"></i> Simpan</button>
								<table>
									<tr>
										<td>
										...
										<td>
											<input type="text" name="nama_unit" id="nama_unit" placeholder="Nama Unit" required oninvalid="this.setCustomValidity('Nama unit harus diisi.')" oninput="setCustomValidity('')" value="<?= $unitTenda["nama_unit"]; ?>" maxlength="100">
										</td>
									</tr>

									<tr>
										<td>
											<label for="harga_unit_1">Harga Dalam Desa</label>
										</td>
										<td>
											<input type="text" name="harga_unit_1" id="harga_unit_1" placeholder="Hara Unit" value="<?= $unitTenda["harga_unit_1"]; ?>">
										</td>
									</tr>

									<tr>
										<td>
											<label for="harga_unit_2">Harga Luar Desa</label>
										</td>
										<td>
											<input type="text" name="harga_unit_2" id="harga_unit_2" placeholder="Hara Unit" value="<?= $unitTenda["harga_unit_2"]; ?>">
										</td>
									</tr>
									...
									<tr>
										...
										<td>
											<input type="text" name="stok_unit" id="stok_unit" placeholder="Stok Unit" required oninvalid="this.setCustomValidity('Stok unit harus diisi.')" oninput="setCustomValidity('')" value="<?= $unitTenda["stok_unit"]; ?>">
										</td>
									</tr>
									...
									<tr>
										<td></td>
										<td>
											<img src="../../../assets/images/kanopi/<?= $unitTenda["gambar_unit"]; ?>" alt="" width="60">
										</td>
									</tr>
									...
									<tr>
										<td></td>
										<td>
											<button type="submit" name="update_tenda" class="btn-green-gradient" onclick="return confirm('Publikasi sekarang?');"><i class="fa fa-save"></i> Simpan</button>
