...
$id = base64_decode(base64_decode(base64_decode(base64_decode(base64_decode(base64_decode($_GET["sejarah-desa"]))))));
$unitDeskripsiDesa = query("SELECT * FROM tb_deskripsi_desa WHERE id = '$id' ")[0];


// URL SECURITY TAMBAHAN 
if( !isset($_GET["sejarah-desa"]) ){
	header("Location:../sejarah-desa");
	return false;
}
if( empty($_GET["sejarah-desa"]) ){
	header("Location:../sejarah-desa");
	return false;	
}
if( strlen($_GET["sejarah-desa"]) < 12 ){
	header("Location:../sejarah-desa");
	return false;	
}
if( !base64_decode(base64_decode(base64_decode(base64_decode(base64_decode(base64_decode($_GET["sejarah-desa"])))))) ){
	header("Location:../sejarah-desa");
	return false;	
}

if( isset($_POST["update-sejarah-desa"]) ){
	if( updateSejarahDesa($_POST) > 0 ){
		echo "
		<script>
			alert('Data Berhasil diubah!');
			document.location.href = '../sejarah-desa';
		</script>";
	} else{
		echo "
		<script>
...
<head>
  ...
  <meta name="description" content="Aplikasi Badan Usaha Milik Desa (BUMDES)" />
	...
	<title>Ubah Data Sejarah Desa</title>
...
<body>
 ...
 -->				<li><a href="../../logout" onclick="return confirm('Pilih Oke jika anda siap untuk mengakhiri sesi anda saat ini.');"><i class="fa fa-sign-out"></i> Keluar</a></li>
		...
		<div class="container-content">
			<div class="header">
				<h2>Ubah Data Sejarah Desa</h2>
			</div>

			<!-- content data sejarah -->
			<div class="container-data-sejarah">
				<div class="card-data-sejarah">
					<div class="menu-data-sejarah">
						<ul>
							<li><a href="../sejarah-desa"><i class="fa fa-angle-left"></i> Kembali</a></li>
						</ul>
					</div>
					<div class="table-form-sejarah">
						<header>Ubah Data Sejarah Desa</header>

						<fieldset>
							<legend>Form Sejarah Desa</legend>
							<form action="" method="post" enctype="multipart/form-data">
								<input type="hidden" name="id" value="<?= $unitDeskripsiDesa["id"]; ?>">
								<input type="hidden" name="posted" value="<?= $rowSession["first_name"]; ?> <?= $rowSession["last_name"]; ?>">
								<input type="hidden" name="gambarLama" value="<?= $unitDeskripsiDesa["gambar"]; ?>">
								<button type="submit" name="update-sejarah-desa" class="btn-green-gradient pd-10 btn-post-members btn-post-sejarah" onclick="return confirm('Publikasi sekarang?');"><i class="fa fa-save"></i> Simpan</button>
								<table>
									<tr>
										<label for="ckeditor-deskripsi">Deskripsi</label>
										<textarea class="ckeditor" name="deskripsi" id="ckeditor-deskripsi"  placeholder="Masukan Info Unit" required oninvalid="this.setCustomValidity('Masukan minimal 10 kata tanggapan anda!.')" oninput="setCustomValidity('')"><?= $unitDeskripsiDesa["deskripsi"]; ?></textarea>
									</tr>
									<br>
									<tr>
										<td></td>
										<td>
											<img src="../../../assets/images/banner/<?= $unitDeskripsiDesa["gambar"]; ?>" alt="" width="60">
										</td>
									</tr>

									<tr>
										<td>
											<label for="gambar">Perbarui Foto <i class="fa fa-image"></i></label>
										</td>
										<td>
											<input type="file" name="gambar" id="gambar" style="cursor: pointer;">
										</td>
									</tr>
									<tr>
									...
									<tr>
										<td></td>
										<td>
											<button type="submit" name="update-sejarah-desa" class="btn-green-gradient" onclick="return confirm('Publikasi sekarang?');"><i class="fa fa-save"></i> Simpan</button>
											<button type="reset" class="btn-danger"><i class="fa fa-refresh"></i> Data Asli</button>
											<a href="../sejarah-desa" class="btn-gray">Batalkan</a>
										</td>
									</tr>
