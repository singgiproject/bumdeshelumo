...
// === QUERY TABLE ORDER (ID URL) ===
// query table order
$idUnit = base64_decode(base64_decode(base64_decode(base64_decode(base64_decode(base64_decode(base64_decode(base64_decode(base64_decode($_GET["unit"])))))))));
$resultUnitOrderId = query("SELECT * FROM tb_order WHERE id = '$idUnit' ")[0];

// === QUERY TABLE TENDA===
// query table tenda
$resultUnitTendaId = $conn->query("SELECT * FROM tb_tenda");
$rowUnitTendaId = mysqli_fetch_assoc($resultUnitTendaId);

// set aritmatika

$hargaUnit = $rowUnitTendaId["harga_unit_1"];
$jumlahUnit = $resultUnitOrderId["jumlah_unit"];
$jangkaWaktu = $resultUnitOrderId["jangka_waktu"];

$hasil1 = $jumlahUnit * $hargaUnit;
$hasilAkhir = $hasil1 * $jangkaWaktu;


// check button submit-save
				...
				<div class="title-content title-content-order">
					<h1>
						<?php if( !empty($rowSessionOrderId) ) : ?>
							Tambah Pesanan <?= $rowSessionOrderId["jenis"]; ?>
						<?php endif; ?>

						<?php if( empty($rowSessionOrderId) ) : ?>
							Tidak Ada Info Pesanan
						<?php endif; ?>
					</h1>
				</div>
						...
						<div class="box-table-order-desktop">
							...
							<table border="1" cellpadding="10" cellspacing="0">
								<?php $id_no = 1; ?>
									...
									<form action="" method="post">
										<input type="hidden" name="id" value="<?= base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode($resultUnitOrderId["id"]))))))))); ?>">
										<tr>
											<td><?= $id_no; ?></td>
											...
											<td>
												<select name="jumlah_unit" id="jumlah_unit" required  oninvalid="this.setCustomValidity('Pilih Jumlah Unit')" oninput="setCustomValidity('')">

													<option value="<?= base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode($resultUnitOrderId["jumlah_unit"])))))))); ?>"><?= $resultUnitOrderId["jumlah_unit"];?> Unit</option>

													<?php for( $jumlah_unit = 1; $jumlah_unit < $rowUnitTendaId["stok_unit"] + 1 - $countStokUnit["orderStokUnit"]; $jumlah_unit++ ) : ?>
											...
											<td>
												<select name="jangka_waktu" id="jangka_waktu" required oninvalid="this.setCustomValidity('Pilih Jangka Waktu')" oninput="setCustomValidity('')">

													<option value="<?= base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode($resultUnitOrderId["jangka_waktu"])))))))); ?>"><?= $resultUnitOrderId["jangka_waktu"];?> Hari</option>
											...
											<td>
												Rp. <?= number_format($rowSessionOrderId["total"], 0, ',', '.'); ?>
												<input type="hidden" name="total" value="<?= base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode($hasilAkhir)))))))); ?>">
												<input type="hidden" value="<?= base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode($rowUnitTendaId["harga_unit_1"])))))))); ?>">
											</td>
											<td>
											...
											<td>
												<button type="submit" name="submit-save" class="submit-save">Simpan</button>
											</td>
										</tr>
									</form>
						...
						<div class="box-table-order-mobile">
							<table cellpadding="10" cellspacing="0">
								...
								<tr>
									<td colspan="2">
											<?php if( $rowSessionOrderId["status"] == 3 ) : ?>
												<a onclick="return confirm('Pemesanan tidak dapat dibatalkan');" id="not-cancel-order-info-mobile"><i class="fa fa-trash" title="Batalkan Pemesanan"></i> Batalkan Pemesanan</a>
											<?php endif; ?>
											...
											<?php if( $rowSessionOrderId["status"] == 0 ) : ?>
												<a href="cancel-order?unit=<?= base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode($rowSessionOrderId["id_user"])))))))); ?>" id="cancel-order-info-mobile" onclick="return confirm('Batalkan Pemesanan?');"><i class="fa fa-trash" title="Batalkan Pemesanan"></i> Batalkan Pemesanan</a>
											<?php endif; ?>
									</td>
								</tr>
							</table>
